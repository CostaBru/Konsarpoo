...
public class MMapData<T> : Data<T>
{
    ...
    public void Load(string filename, int sizeOfArray)
    {
        MemoryMappedDataVariableSizeSerializationInfo info = MemoryMappedDataVariableSizeSerializationInfo.Open(filename, sizeOfArray);

        (int maxSizeOfArray, int dataCount, int version, int arraysCount) metaData = info.ReadMetaData();
    ...
    private class MMapStoreNode : INode
    {
        public IArrayAllocator<T> Allocator { get; }
        ...
        {
            node = new MMapStoreNode(m_parent, Allocator, array, size);
            return false;
        }

        public MMapStoreNode(INode parent, IArrayAllocator<T> allocator, int maxCapacity, int capacity)
        {
            Allocator = allocator;
            m_parent = parent;
        }

        public MMapStoreNode(INode parent, IArrayAllocator<T> allocator, T[] items, int size)
        {
            m_parent = parent;
            m_size = size;
        }

        public MMapStoreNode(INode parent, StoreNode poolList)
        {
            m_parent = parent;
        }

        public MMapStoreNode(INode parent, IArrayAllocator<T> allocator, int maxCapacity, int capacity, T item)
        {
            m_parent = parent;
        ...
        public bool Ensure(ref int extraSize, ref T defaultValue, out INode node)
        {
                ...
                //allocate new with rest
            {
                ...
                if (ArrayAllocator.CleanArrayReturn == false ||
    ...
    private class MMapLinkNode : INode
    {
        ...
        public MMapLinkNode(INode parent, int level, int leafCapacity, INode child1, IArrayAllocator<INode> nodesAllocator,
            INode child2 = null)
        {
            m_parent = parent;
            m_level = level;
        ...
        public MMapLinkNode(INode parent, MMapLinkNode linkNode)
        {
            m_parent = parent;
        ...
        public bool AddArray(T[] array, int size, out INode node)
        {
            if (m_nodes[m_nodes.m_size - 1].AddArray(array, size, out var node1) == false)
            {
                if (m_nodes.m_size == c_linkNodeCapacity)
                {
                    node = new MMapLinkNode(m_parent, m_level, m_leafCapacity, node1, m_nodesAllocator);
                    return false;
                }
        ...
        public IEnumerator<T> GetEnumerator()
        {
            foreach (INode node in m_nodes)
            {
                if (node is MMapStoreNode storeNode)
                {
